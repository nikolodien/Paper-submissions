...
phrases. Topic models in general don't work properly with phrases. To tackle 
this situation, we propose an approach to sentiment analysis using topical 
n-grams which have been shown to be effective with phrases. We train the topical 
n-grams model using two topics i.e; positive and negative, list of positive 
and negative words, and rules to detect positive and negative phrases. New 
documents are then classified using this trained model. The system gives 
better results than the existing Joint Sentiment Topic model. We also propose 
an approach to generate list of positive ang negative words using LDA.

\end{abstract}

\section{Introduction}

LDA (Latent Dirichlet Allocation) as shown in ~\cite{blei2003latent} is generative 
model used to discover topics in a document collection. It gives two types of 
...
to the same topic. JST (Joint Sentiment Topic) as explained in ~\cite{lin2009joint} 
...
of words. Not all collocations of words can be considered as a phrase. A model which
takes into account phrases is needed to tackle this situation.

Topical n-grams ~\cite{wang2007topical} is one such generative model.
